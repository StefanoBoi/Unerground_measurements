...
void rate_depth()
{
  ...
  Float_t rate[N] = {3.831, 0.00542, 0.004, 0.001131, 0.0007783}; //from fit
  Float_t rate_err[N] = {0.04, 0.0001, 0.000054, 0.00004,  0.000012};
  Float_t depth_err[N] = {0, 5, 5, 5, 5};
  Float_t density[N];// = {1, 2.2126, 2.0619, 1.8923};//2.00825}; //density on 339->512 1.67 or 1.56
  Float_t depth_mwe[N];
  
  Float_t Astro_rate[N] = {8.618, 0.01257, 0, 0.002556, 0.001835};
  ...
  for(int i = 0; i < N; i++)
    {
      ...
      cout << depth[i] << "   " << depth_mwe[i] << "   " << rate_norm[i] << " +/- " << rate_norm_err[i] << "   " << Astro_rate_norm[i] << endl;
    }
  cout << endl << endl;
  ...
  TGraph *Astro_rate_depth = new TGraphErrors(N, depth, Astro_rate);
  TGraph *Astro_rate_depth_suppr = new TGraphErrors(N, depth, Astro_rate_suppr);
  TGraph *Astro_rate_depth_norm = new TGraphErrors(N, depth, Astro_rate_norm);
  ...
  c4->SetGridx();
  c4->SetGridy();
  TGraph *norm_mwe = new TGraph(N, depth_mwe, rate_norm);
